...
The PLM treats the noise in a \Kepler\ pixel $m$ at time $n$ as Gaussian (normally distributed)
  with mean zero and known variance $\sigma^2_{mn}$,
  and the expectation value for each \Kepler\ data point $I_{mn}$ as being
...
This linear model has parameters (linear coefficients) $a_{mm'}$.
\todo{foo:}
\begin{eqnarray}
...
\\
p(e_{mn})      &=& N(e_{mn}\given 0,\sigma^2_{mn})
\quad,
\end{eqnarray}
where HOGGGGGGGGGGGGGGGGGGGGGGGG
\begin{eqnarray}
\chi^2_{mn}    &=& \sum_{n'\in\set{N}_n} \frac{[I_{mn'} - I^{\ast}_{mn'}]^2}{\sigma^2_{mn'}}
                 + \sum_{m'\in\set{M}_m} \frac{a_{mm'}^2}{\Sigma^2}
\\
I^{\ast}_{mn'} &=& \sum_{m'\in\set{M}_m} a_{mm'}\,I_{m'n'}
\quad,
\end{eqnarray}
where \todo{bar}
