...
(For intensity measurements $I_{mn}$ here we are using pixels from the \todo{WHICH \Kepler\ image product?})
...
\todo{Issue arising: I have called (below) the coefficients $a_{mm'}$.
However, they are different for every pixel $I_{mn}$.
That is, we do a separate optimization for every read-out pixel.
That means that really we should call the coefficients $a_{mnm'}$, right?
DFM:  Comments?  The same issue arises for the predictions $I^{\ast}_{mn'}$;
  these predictions are specific to $I_{mn}$ (through the coefficients) and therefore should be $I^{\ast}_{mnn'}$}
...
\\
I^{\ast}_{mn}  &=& \sum_{m'\in\set{M}_m} a_{mm'}\,I_{m'n} + \sum_{k=1}^{K} b_{mk}\,d_{kn}
\quad,
\end{eqnarray}
where $I^{\ast}_{mn}$ is the prediction (by the model) for data point $I_{mn}$,
  ...
  and the $a_{mm'}$ and $b_{mk}$ are the parameters (linear coefficients of the prediction).
...
We add to the standard chi-squared definition a regularization term
  ...
  that penalizes large absolute values for the the coefficients $a_{mm'}$:
\begin{eqnarray}
\chi^2_{mn}    &=& \sum_{n'\in\set{N}_n} \frac{[I_{mn'} - I^{\ast}_{mn'}]^2}{\sigma^2_{mn'}}
                 + \sum_{m'\in\set{M}_m} \frac{a_{mm'}^2}{\Sigma^2}
\\
I^{\ast}_{mn'} &=& \sum_{m'\in\set{M}_m} a_{mm'}\,I_{m'n'} + \sum_{k=1}^{K} b_{mk}\,d_{kn'}
\quad,
\end{eqnarray}
...
\todo{DFM: We set or ``know'' the variances how?
  I think we should use the inverse of the median of the reported inverse variances.
  Also, we set the prior strength to what value?
  Heuristic justification of the latter.}
...
We train the model---that is, set the parameters $a_{mm'}$ and $b_{mk}$---%
  by choosing the full set of parameters that jointly minimize the objective function $\chi^2_{mn}$.
Importantly, and perhaps surprisingly, this is a \emph{different objective function}
  for every target data point (pixel value to be predicted) $I_{mn}$,
  and we have to do a \emph{independent optimization} of the parameters for every pixel value we want to predict.
That is, every pixel datum $I_{mn}$ we predict will have
  \emph{different} settings of the parameters $a_{mm'}$ and $b_{mk}$.
Since there are about $10^{11}$ total pixel values in the \Kepler\ \todo{what imaging?} data archive,
  this represents one fuck of a lot of optimizations,
  and a lot of parameters to obtain and record.

For this reason---\emph{so many optimizations}---%
  we do \todo{some crazy clever stuff} to make this work fast;
  \todo{DFM implementation notes here}.

Once we have obtained the $a_{mm'}$ parameters corresponding to data point $I_{mn}$ by minimizing the training objective $\chi^2_{mn}$.

\todo{HOGG: We construct the predicted images and the \name\ images how?}
