...
We call this set of pixels%
  ---from the $Q$ stars on the same CCD---%
  the \emph{pixel set} $\set{M}_m$ associated with pixel $m$.
...
That is, we train (optimize fit parameters) using a set $\set{N}_n$ of time
  indices $n'$ such that for all $n'\in\set{N}_n$,
  $t_{n'}$ is in the same quarter as $t_n$,
  and $|t_{n'} - t_n|>\Delta t$.
The training set of time indices $\set{N}_n$ therefore does not overlap the time point $t_n$,
  nor any of its neighbors in a time window of half-width $\Delta_t$.

\begin{eqnarray}
I_{mn}         &=& I^{\ast}_{mn} + \mbox{noise}
\\
