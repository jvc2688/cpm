...
\remove{We propose and advise dealing with}\revise{We propose to militate} these variations in \Kepler\ light-curves by \emph{modeling} them.
...
In \revise{the} \name, systematics and stellar variabilities are removed by
  either fitting with other stars' light-curves or auto-regressive components, while transit signals \remove{can still be perfectly preserved} \revise{are preserved with a} \remove{by} train-and-test framework.
